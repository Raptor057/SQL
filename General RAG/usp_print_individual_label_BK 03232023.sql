...
DECLARE
		...
		@ratio NVARCHAR(50),		-- ?
		@origen NVARCHAR(50),		-- Get Origen from CEGID 
		@p1 nvarchar(50),			-- Get Patent from CEGID
		@p2 nvarchar(50),			-- Get Patent from CEGID
		@p3 nvarchar(50),			-- Get Patent from CEGID
		@p4 nvarchar(50),			-- Get Patent from CEGID
		@p5 nvarchar(50),			-- Get Patent from CEGID
		@p6 nvarchar(50),			-- Get Patent from CEGID
		@p7 nvarchar(50),			-- Get Patent from CEGID
		@p8 nvarchar(50),			-- Get Patent from CEGID
		--@is_ME_FL BIT,
		@is_double_lid BIT;			-- Indicates whether double print is required
	...
	--set @origen =(SELECT top 1 * FROM ufn_GetArticleOrigenCegid(@part_no,SUBSTRING(@part_rev, 1, 1))) --Get Origen by part_no and part_rev directly of CEGID
...
--=================================================================================
/*Esto es un mal truco, ya que dezconozco desde donde se escribe el valor type en la tabla APPS.dbo.pro_tms
asi que el truco actuda de manera que si no detecta nincuna patente existente enconces deja el tipo original, en caso de que detecte alguna patente,
entonces la etiqueta es de tipo ME FL*/
		declare @is_ME_FL BIT;
		IF COALESCE(@p1, '') = '' 
BEGIN
    SET @is_ME_FL = 0;
END
...
--=================================================================================
	...
	SET @Body = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(@Body,
						...
						'{lineCode}', @line),
						'{reference}', @reference),
						-- 2022-22-04 RA: Added Origen From CEGID with ussing a function GetArticleOrigenCegid.
						'{origen}', ISNULL(NULLIF(RTRIM(@origen), ''), ' ')),
						-- 2023-23-04 RA: Added patent From CEGID and APPS with ussing a function Get[atentsByPartNoAndRev.
						'{patente1}', ISNULL(NULLIF(RTRIM(@p1), ''), ' ')),
						'{patente2}', ISNULL(NULLIF(RTRIM(@p2), ''), ' ')),
						'{patente3}', ISNULL(NULLIF(RTRIM(@p3), ''), ' ')),
						'{patente4}', ISNULL(NULLIF(RTRIM(@p4), ''), ' ')),
