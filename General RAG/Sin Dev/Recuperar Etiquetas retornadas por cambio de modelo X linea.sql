...
WHERE PointOfUseCode LIKE (CONCAT('%',@Line,'%')) --AND EtiNo = @EtiNo AND @ComponentNo = @ComponentNo
...
--AND UtcExpirationTime = '' -- Cuando ya se tenga la fecha del tiempo de descarga ponerlo aqui para ver realmente cuantos componentes se tendrian que haber descargado
...
SELECT PointOfUseCode,EtiNo,ComponentNo,UtcExpirationTime FROM [gtt].[dbo].[PointOfUseEtis] WHERE IsDepleted = 0 GROUP BY PointOfUseCode,EtiNo,ComponentNo,UtcExpirationTime ORDER BY PointOfUseCode ASC, UtcExpirationTime DESC