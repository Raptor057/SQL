CREATE PROCEDURE InsertComponentsSnapshot
...
BEGIN
    ...
    SELECT
        CAST(FORMAT(GETUTCDATE(), 'yyyyMMddHHmmssfff') AS BIGINT) AS [ID],
        LPU.LineCode,
        PUE.PointOfUseCode,
        PUE.EtiNo,
        CASE
        ...
        END AS MasterEtiNo,
        PUE.ComponentNo,
        RTRIM(CB.NOCTCOMCPT) AS [ComponentRev],
        RTRIM(CB.ARCTLIB01) AS [Description],
        PUE.LotNo
    FROM
        [gtt].[dbo].[PointOfUseEtis] PUE
        ...
        RIGHT JOIN [MXSRVTRACA].[TRAZAB].[cegid].[bom] CB ON
            RTRIM(CB.NOKTCODPF) COLLATE SQL_Latin1_General_CP1_CI_AS = LPS.PartNo
    ...
    WHERE
        LPU.LineCode = @LineCode
        AND PUE.UtcExpirationTime IS NULL
        AND PUE.UtcUsageTime IS NOT NULL
        AND PUE.UtcUsageTime < GETUTCDATE()
END
